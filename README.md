...
- Keep the R2 live smoke in the release gate. The current implementation has successfully uploaded a correctly signed multipart part and rejected a wrong same-length payload, but production bindings still need environment-specific verification after deployment.
