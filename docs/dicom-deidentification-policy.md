...
- **Philips classic:** the client may retain only bounded VM1 physical values for number of slices `(2001,xx18)`, water-fat shift `(2001,xx22)`, and scale intercept/slope `(2005,xx0D/xx0E)` under their exact canonical creators. No other value in those blocks is retained. The private dynamic-scan-begin value `(2005,xxA0)` is local-only evidence and is never serialized. Public `TriggerTime (0018,1060)` is suppressed only when every expected instance proves a zero-based, TR-multiple dynamic sequence under the complete series-level timing contract; malformed, incomplete, shifted, or nonredundant timing holds the series. The archive manifest records that transformation.
- **Philips Enhanced:** a bounded implementation exists for rebuilding the per-frame scale sequence to its creator plus VM1 slope only, but all Enhanced MR objects remain held until the complete shared/per-frame scientific metadata contract has conversion-equivalence evidence.
- **GE classic:** all private values remain excluded and the route is held as `ge_classic_requires_verified_private_metadata_reconstruction`. A fixture-proven reconstruction design exists, but it is not enabled until its bounded parser/rebuilder and hostile tests are production-ready.

Extended Offset Table elements are also held pending exact validation of their relationship to encapsulated Pixel Data. The cluster conversion result is a derived validation artifact and never retroactively makes an unsafe local archive acceptable.

## Burned-in pixels

This EPI-only beta rejects Secondary Capture, derived presentation images, overlays, and graphics before packaging. An explicit `BurnedInAnnotation (0028,0301)` value other than `NO` is rejected. For the two currently measured classic-MR routes only—Siemens Prisma_fit/E11 and Philips Achieva dStream/5.1.1—an absent attribute may proceed only when every otherwise eligible image is `ORIGINAL` and `PRIMARY`; the client does not synthesize `NO`, and the manifest records `not_declared`. This is a bounded direct-acquisition heuristic, not proof that a PACS never changed the pixels and not a rule automatically extended to another scanner route. The client does not inspect or alter brain-image pixels for text. Structural imaging remains out of scope until a separate face-privacy path is implemented and validated.
