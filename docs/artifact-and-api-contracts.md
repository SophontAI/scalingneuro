...
Before upload, the client writes a local preparation manifest conforming to `local-manifest-v1.schema.json`. This owner-only file deliberately contains staging paths and therefore is never uploaded, logged, or used as the shareable run report. It records the enrolled `site_id`, `project_id`, and `consent_policy_version`; resume fails closed unless all three still match the current protected enrollment configuration.
...
Clients may retry network failures, `CREDENTIALS_UNAVAILABLE` (the stable v1 code for a temporarily unavailable part signer), `STORAGE_UNAVAILABLE`, and ordinary 429/5xx responses with bounded exponential backoff. After a part URL expires, they request a new URL for the same allocated part. `INVALID_REQUEST`, consent-policy updates, revocation, object mismatch, and other deterministic 4xx states require a local state transition or user-visible action instead of a blind retry loop.
