...
fn dry_run_prepares_privacy_cleared_dicom_archives_without_network() {
    ...
    assert!(progress_output.contains("Preparing privacy-cleared EPI archives"));
    ...
    assert!(
        report["errors"]
            ...
            .unwrap()
            .iter()
            .any(|value| value == "unreadable_dicom_like_files")
    );
    ...
    assert!(expanded.contains("scaling-neuro-recursive-allowlist-v1"));
}
