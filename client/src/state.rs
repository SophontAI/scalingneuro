...
use crate::{dicom::SourceFingerprint, model::SourceSummary, privacy};
...
impl StateStore {
    ...
    /// existing row is important: accepting an incompatible checkpoint could
    /// associate a receipt with the wrong local archive.
    pub fn ensure_single_series_upload(
        ...
        if row.bundle_start != bundle_index || row.bundle_count != 1 {
            anyhow::bail!("existing DICOM upload checkpoint uses an incompatible archive layout");
        }
        Ok(row)
    }
